 ...
 *
 * Steps:
 *   5. Runs tests + build as a safety gate.
 *   6. Consumes changesets to bump package.json + CHANGELOG.md.
 *   7. Commits the result as "Version X.Y.Z".
...
// 7. Commit. Stage only the files changeset version touches.
const { version } = JSON.parse(readFileSync("./package.json", "utf-8"));
run("git add package.json CHANGELOG.md .changeset");
run(`git commit -m "Version ${version}"`);

// 8. Show the commit and prompt.
console.log("");
run("git --no-pager show HEAD --stat");
console.log("");
console.log("-".repeat(60));
console.log(`  About to release scriptable v${version}`);
console.log("-".repeat(60));
...
// 9. Push main, tag, push tag.
run("git push origin main");
run("pnpm exec changeset tag");
run("git push --follow-tags");

console.log(`\nv${version} tag pushed.`);
console.log(`  https://github.com/lecstor/scriptable/actions`);
